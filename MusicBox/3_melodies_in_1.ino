...
// these are the songs we're going to play; we assign each song name a number.
// we'll use these to keep track of which song is next to be played.
#define CHARGE            1
#define FUR_ELISE         2
#define IMPERIAL_MARCH    3
...
// the "melody" variable specifies which song should be played next.
// because it's defined here, outside any function, it has global scope
// (which means it's accessible throughout the whole sketch).
// we'll start with the shortest, simplest melody -- "CHARGE"
int melody = CHARGE;
...
void loop(){
  ...
  if (digitalRead(PIN_BUTTON) == LOW){
    ...
    // based on the current value of the "melody" variable, choose a
    // song to play. after it's done playing, select the next song.
...
void play_charge(){
  ...
  // variable "i" is our position within the "charge" structure
  for (int i=0; i<note_count; i++){

    // if this particular note isn't SILENCE, turn on the LED and emit a tone at FREQUENCY
    if (charge[i][FREQUENCY] != SILENCE){
      digitalWrite(PIN_BUILTIN_LED, HIGH);
      tone(PIN_BUZZER, charge[i][FREQUENCY]);
    }
    // wait for the prescribed DURATION to elapse
    delay(charge[i][DURATION]);
